...
void get_parameter_decoder(uint32_t position, uint8_t datum)
{
	const char* parameter = "1,Spannung,f,f,0,5,{0,3.3,5}";
	uint32_t size = strlen(parameter);
	if (position==size+5) decoderbytenr=0;
	else if (position==0) send_spi(comhandlerspi,size>>24);
	else if (position==1) send_spi(comhandlerspi,size>>16);
	else if (position==2) send_spi(comhandlerspi,size>>8);
	else if (position==3) send_spi(comhandlerspi,size);
